...
switch ($_GET['act']) {
  ...
  default:
    if ($_POST['submit']) {
      ...
      // $arbobot = array('0', '1', '0.8', '0.6', '0.4', '-0.2', '-0.4', '-0.6', '-0.8', '-1'); // lama
        ...
        while ($rgejala = mysqli_fetch_array($sqlgejala)) {
          ...
          for ($i = 0; $i < count($_POST['kondisi']); $i++) {
            ...
            if ($rgejala['kode_gejala'] == $gejala) {
              ...
              if (($cf >= 0) && ($cf * $cflama >= 0)) {
                $cflama = $cflama + ($cf * (1 - $cflama)); // cf combine
              }
              if ($cf * $cflama < 0) {
...
// --------------------- END -------------------------
	...
	<h2 class='text text-primary'>Hasil Diagnosis &nbsp;&nbsp;<button id='print' onClick='window.print();' data-toggle='tooltip' data-placement='right' title='Klik tombol ini untuk mencetak hasil diagnosa'><i class='fa fa-print'></i> Cetak</button> </h2>
      ...
      echo "<div class='callout callout-default'>Jenis penyakit yang diderita adalah <b><h3 class='text text-success'>" . $nmpkt[1] . "</b> / " . round($vlpkt[1]*100, 2) . " % (" . $vlpkt[1] . ")<br></h3>"; // kodingan awal round tidak di kali 100
      ...
      // kodingan awal for ($ipl = 2; $ipl < count($idpkt); $ipl++) { 
      for ($ipl = 2; $ipl <= count($idpkt); $ipl++) { 
        echo " <h4><i class='fa fa-caret-square-o-right'></i> " . $nmpkt[$ipl] . "</b> / " . round($vlpkt[$ipl]*100, 2) . " % (" . $vlpkt[$ipl] . ")<br></h4>"; // kodingan awal round tidak di kali 100
      }
      echo "</div></div>
		  </div>";
    } else {
	 ...
	 <div class='alert alert-success alert-dismissible'>
                ...
                Silahkan memilih gejala sesuai dengan kondisi yang dialami oleh anda, anda dapat memilih kepastian kondisi yang dialami dari pasti tidak sampai pasti ya. jika sudah, maka tekan tombol proses (<i class='fa fa-search-plus'></i>)  di bawah untuk melihat hasil.
              </div>
